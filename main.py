    ...
    async with aiosqlite.connect(DB) as db:
        await db.executescript('''
            CREATE TABLE IF NOT EXISTS users (
                user_id INTEGER PRIMARY KEY, username TEXT, trial_end TEXT,
                premium_until TEXT, premium_type TEXT, referred_by INT,
                referred_count INT DEFAULT 0, push_answers INT DEFAULT 1,
                theme TEXT DEFAULT "dark", accent_color TEXT DEFAULT "#8774e1"
            );
            CREATE TABLE IF NOT EXISTS questions (
                id INTEGER PRIMARY KEY AUTOINCREMENT, from_user INT, to_user INT,
                text TEXT, answer TEXT, answered INT DEFAULT 0,
                hidden INT DEFAULT 0, special INT DEFAULT 0, likes INT DEFAULT 0,
                bumped_at TEXT, created_at TEXT DEFAULT (datetime('now'))
            );
...
# === СТАРТ + РЕФЕРАЛКА (+1 день за друга) ===
...
async def get_user(m: types.Message, state: FSMContext):
    ...
    async with aiosqlite.connect(DB) as db:
        ...
        if not row:
            await m.answer("Пользователь ещё не в боте — уведомим при старте!", reply_markup=main_kb())
            await state.clear(); return
        celeb = await (await db.execute("SELECT name FROM celebs WHERE user_id=?", (row[0],))).fetchone()
        if celeb:
            await m.answer(f"Вопрос {celeb[0]} — 250⭐", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [{"text": "Оплатить 250⭐", "pay": True}]
            ]))
            await state.update_data(to_id=row[0], cost=250)
        else:
...
async def type_sel(c: types.CallbackQuery, state: FSMContext):
    ...
    await state.update_data(special=special, cost=5 if special else 0)
    await c.message.edit_text("Напиши вопрос:")
    await state.set_state(Ask.question)

@dp.message(Ask.question)
async def send_question(m: types.Message, state: FSMContext):
    data = await state.get_data()
    to_id, special, cost = data["to_id"], data.get("special",0), data.get("cost",0)
    async with aiosqlite.connect(DB) as db:
    ...
    await m.answer("Отправлено!", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton("Ещё", callback_data="ask")],
        [InlineKeyboardButton("Кабинет", web_app=WebAppInfo(url=f"{BASE_URL}/miniapp"))]
    ]))
    await state.clear()

@dp.message()
async def reply_like(m: types.Message):
    if m.reply_to_message and "Новый вопрос" in m.reply_to_message.text:
        ...
        async with aiosqlite.connect(DB) as db:
            ...
            if row:
                await db.execute("UPDATE questions SET answer=?,answered=1 WHERE from_user=? AND to_user=? AND text=?",
                               (m.text,row[0],m.from_user.id,qtext))
                await db.commit()
                await bot.send_message(row[0],f"Ответ:\n\n{m.text}")
                await m.answer("Отправлено!",reply_markup=main_kb())
    if m.text in ["❤️","♥️"] and m.reply_to_message:
        async with aiosqlite.connect(DB) as db:
            await db.execute("UPDATE questions SET likes=likes+1 WHERE id=?", (m.reply_to_message.message_id,))
            await db.commit()
        await m.answer("❤️")
        # === ОПЛАТА ЗВЁЗДАМИ ===
...
async def stars_paid(m: types.Message):
    ...
    days = {135:30, 330:90, 1050:365, 2600:99999, 250:0, 5:0, 10:0}.get(amount, 0)
    if days:
        end = "9999-12-31" if days > 365 else (datetime.now(timezone(timedelta(hours=3))) + timedelta(days=days)).strftime("%Y-%m-%d")
        async with aiosqlite.connect(DB) as db:
...
async def export_pdf(c: types.CallbackQuery):
    async with aiosqlite.connect(DB) as db:
        rows = await (await db.execute("SELECT text,answer FROM questions WHERE from_user=? OR to_user=? ORDER BY id DESC", (c.from_user.id,c.from_user.id))).fetchall()
    buffer = BytesIO()
    p = canvas.Canvas(buffer, pagesize=A4)
    w, h = A4; y = h - 50
    for q,a in rows:
        p.drawString(50, y, f"Q: {q[:100]}"); y -= 20
        if a: p.drawString(70, y, f"A: {a[:100]}"); y -= 20
        if y < 100: p.showPage(); y = h - 50
    p.save(); buffer.seek(0)
    await bot.send_document(c.from_user.id, InputFile(buffer, "questions.pdf"))
    await c.answer("PDF отправлен!")
...
async def admin(m: types.Message):
    ...
    async with aiosqlite.connect(DB) as db:
        stats = await (await db.execute("""SELECT
            ...
            premium_until
        """,(uid,)*5)).fetchone() or (0,0,0,0,None)

    top = ""
    async with aiosqlite.connect(DB) as db:
        rows = await (await db.execute("SELECT u.username,COUNT(*)FROM questions q JOIN users u ON q.to_user=u.user_id GROUP BY q.to_user ORDER BY COUNT(*) DESC LIMIT 10")).fetchall()
        for i,(u,c) in enumerate(rows,1): top += f"{i}. @{u} — {c}<br>"
    ...
    button{{margin:10px 0;padding:16px;width:90%;background:var(--tg-theme-button-color);color:var(--tg-theme-button-text-color);border:none;border-radius:16px;font-size:18px}}
    </style></head><body>
    <h1>Личный кабинет</h1>
    <div class="c"><div class="n">{stats[0]}</div>Отправлено</div>
    <div class="c"><div class="n">{stats[1]}</div>Получено</div>
    <div class="c"><div class="n">{stats[2]}</div>Отвечено</div>
    <div class="c"><div class="n"style="color:#e74c3c">{stats[3]}</div>Ждут ответа</div>
    <div class="c">Премиум до: <b>{stats[4] or "Нет"}</b></div>
    <button onclick="Telegram.WebApp.openInvoice('stars',{{title:'1 месяц',payload:'m',prices:[{{label:'135⭐',amount:135}}]}})">135⭐ — 1 месяц</button>
    <button onclick="Telegram.WebApp.openInvoice('stars',{{title:'3 месяца',payload:'3m',prices:[{{label:'330⭐',amount:330}}]}})">330⭐ — 3 месяца</button>
    <button onclick="Telegram.WebApp.openInvoice('stars',{{title:'Год',payload:'y',prices:[{{label:'1050⭐',amount:1050}}]}})">1050⭐ — год</button>
    <button onclick="Telegram.WebApp.openInvoice('stars',{{title:'Пожизненно',payload:'life',prices:[{{label:'2600⭐',amount:2600}}]}})">2600⭐ — навсегда</button>
    <h3>Топ-10</h3><div style="font-size:14px">{top or "Пока пусто"}</div>
